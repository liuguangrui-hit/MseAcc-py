...
def train(model, train_loader, epoch_):
    ...
    print('Train Epoch: {}\t Acc: {}/{} ({:.6f}%)\t Loss: {:.6f}'.format(epoch_ + 1, correct, cnt, 100. * correct / cnt,
                                                                         loss_mean.item()))

    file_handle.write(
        'Train Epoch: {}\t Acc: {}/{} ({:.6f}%)\t Loss: {:.6f}\n'.format(epoch_ + 1, correct, cnt, 100. * correct / cnt,
                                                                         loss_mean.item()))
    file_handle.close()
    mse_handle.close()
    return max(mse_list)
...
def roll(epoch_, val_dl):
    try:
        ...
        val_min_list = []
        for i in range(5):
            model_p = "saved_models/DNN/{}_{}_NIDS_DNN.pt".format(epoch_ - 1,i)
        ...
        if min(val_min_list) > 0.92:
            ...
            torch.save(state, "saved_models/NIDS_DNN.pt")
            # return start_epoch
        else:
...
epoch = 10
val_path = 'saved_models/val_DNN.txt'
model_path = 'saved_models/DNN/'
mse_path = 'saved_models/mse_DNN.txt'
log_path = 'saved_models/log_DNN.txt'
...
if __name__ == '__main__':
    ...
    #                      batch_size=32, shuffle=False)
    val_dl = DataLoader(Dataset_adv_1('val_data/val_set_DNN.csv'),
                        batch_size=32, shuffle=False)
    ...
    if not reuse_model and is_train:
        ...
        while i < epoch:  # 训10个epoch
            ...
            for j in range(5):
                ...
                torch.save(state, model_path + "{}_{}_NIDS_DNN.pt".format(i, j))
                torch.save(state, "saved_models/NIDS_DNN.pt")
            ...
            if (i > 2) and ((max(mse_max_list) > mse_max_value2) or (min(val_min_lsit) < val_min_value1)):
                ...
                if c == 'e' or c == 'E':
                    ...
                    roll(i, val_dl)
                    model_p = "saved_models/NIDS_DNN.pt"
                    if os.path.exists(model_p):
                        checkpoint = torch.load(model_p)
                        net.model.load_state_dict(checkpoint['model'])
                        optimizer.load_state_dict(checkpoint['optimizer'])
    ...
    # test
        ...
        while not loop_exit:
            ...
            if c == '2':
                # test val
                test_dl = DataLoader(Dataset_adv_1('val_data/val_set_DNN.csv'),
                                     batch_size=32, shuffle=False)
                loop_exit = True
            if c == '3':
                # test bypass
                test_dl = DataLoader(
                    Dataset_adv_1("../data/cic_2017/adver_sets/1.0_DNN_adver_example.csv"), batch_size=32,
                    shuffle=False)
                loop_exit = True
        # model_p = "saved_models/DNN/0_4_NIDS_DNN.pt"
        model_p = "saved_models/NIDS_DNN.pt"
        if os.path.exists(model_p):
            checkpoint = torch.load(model_p)
            net.model.load_state_dict(checkpoint['model'])
