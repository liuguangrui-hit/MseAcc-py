...
def train(model, train_loader, epoch, is_end):
    ...
    for data in train_loader:
        ...
        # print(target)
        if is_end:
            # with open("data/MLP_NIDS_result.csv", "a") as f:
    ...
    if is_end:
        ...
        np.save('data/time1_fail_MLP.npy', fail_samples)
        print("success_samples size:", len(success_samples))
        success_samples = np.array(success_samples)
        np.save('data/time1_success_MLP.npy', success_samples)
    loss_mean = train_loss / (i + 1)
    # print(f"准确率:{correct / cnt}")
    print('Train Epoch: {}\t Acc: {}/{} ({:.6f}%)\t Loss: {:.6f}'.format(epoch + 1, correct, cnt, correct / cnt,
                                                                         loss_mean.item()))
...
if __name__ == '__main__':
    ...
    if not reuse_model and is_train:
        ...
        for i in range(epoch):  # 训10个epoch
            ...
            # torch.save(state, model_path + str(k) + "_epoch5_NIDS_MLP.pt")
            k += 0.1
